                ...
                .add_byte(byte, |command, writer| match command {
                    CommandGroup::Base(base_command) => match base_command {
                        ...
                        BaseCommands::Int(value) => {
                            let _ = writer.write_line(format!("{:?}", value).as_bytes());
                        }
                        ...
                        BaseCommands::Compound((value1, value2)) => {
                            let _ =
                                writer.write_line(format!("{:?}, {:?}", value1, value2).as_bytes());
                        }
                        BaseCommands::List => {
                            let rows: &[&[&str]] = &[&["Pin A", "30"], &["Pin B", "42"]];
                            let _ = writer.write_list::<256>(rows, true);
                        }
                        BaseCommands::Table => {
                            let headers = ["Name", "Value"];
                            let rows: &[&[&str]] = &[&["Pin A", "30"], &["Pin B", "42"]];
                            let _ = writer.write_table::<256>(&headers, rows);
                        }
                        BaseCommands::SomeJson { json, pretty } => {
                            if let Some(true) = json {
                                let _ =
                                    writer.write_json::<512>(&some_json, pretty.unwrap_or(false));
                            } else {
                                let _ = writer.write_line(format!("{}", some_json).as_bytes());
                            };
                        }
                        BaseCommands::Command {
                            ...
                            let _ = writer.write_line(
                                format!("{:?} {:?} {:?}", name, name2, some_flag).as_bytes(),
                            );
                        }
                    ...
                    CommandGroup::Second(command) => match command {
                        SecondCommandGroup::Simple2 => {
                            let _ = writer.write_line("Second simple command".as_bytes());
                        }
                    },
                })
